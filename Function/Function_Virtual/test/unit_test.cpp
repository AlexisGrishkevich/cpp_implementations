...
// member function //---------------------------------------------------//
TEST(Function, initialization_from_member_function) {
  S s;
  Function<int(S&, int)> function{&S::bar};

  ASSERT_EQ(function(s, 2), 2);
}

TEST(Function, initialization_from_member_function_move) {
  S s;
  Function<int(S&, int)> function{std::move(&S::bar)};

  ASSERT_EQ(function(s, 2), 2);
}

TEST(Function, copy_assignment_from_member_function) {
  S s;
  Function<int(S&, int)> function;
  function = &S::bar;

  ASSERT_EQ(function(s, 2), 2);
}

TEST(Function, move_assignment_from_member_function) {
  S s;
  Function<int(S&, int)> function;
  function = std::move(&S::bar);
